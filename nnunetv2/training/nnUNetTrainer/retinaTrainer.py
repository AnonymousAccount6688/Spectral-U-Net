...
class RetinaTrainer(nnUNetTrainer):
    ...
    def build_network_architecture(plans_manager: PlansManager,
        ...
        return get_network_from_plans(plans_manager, dataset_json, configuration_manager,
                                      ...
                                      base_ch=RetinaTrainer.base_ch,
                                      block=RetinaTrainer.block,
                                      use_my_unet=RetinaTrainer.use_my_unet,
                                      setting=RetinaTrainer.setting)
    ...
    def real_validation_retina(self):
        ...
        all_hd95_mean = np.nanmean(all_hd95, axis=0)

        print("all_hd95_mean", all_hd95_mean)
        ...
        if self.local_rank == 0:
            ...
            if not self.debug:
                ...
                wandb.log(data={"test/hd95_IRF": all_hd95_mean[0]}, step=self.current_epoch)
                wandb.log(data={"test/hd95_SRF": all_hd95_mean[1]}, step=self.current_epoch)
                wandb.log(data={"test/hd95_PED": all_hd95_mean[2]}, step=self.current_epoch)
    ...
    def initialize(self):
        # self.initialize_wandb(project=self.dataset_name, name=self.dataset_name)
        if not self.was_initialized:
    ...
    def save_checkpoint(self, filename: str) -> None:
        if self.local_rank == 0:
            if not self.disable_checkpointing:
                ...
                self.print_to_log_file(f"saved to {filename}")
            else:
