...
from nnunetv2.utilities.liver_get_network_from_plans import get_network_from_plans
...
class LiverTrainer(nnUNetTrainer):
    ...
    def __init__(self, plans: dict, configuration: str, fold: int, dataset_json: dict, unpack_dataset: bool = True,
        ...
        #
        # self.output_folder_base = f"/scratch365/ypeng4/data/result/{self.dataset_name}/"\
        #                           f"{configuration}/wave_unet"

        self.output_folder_base = f"/scratch365/ypeng4/data/result/{self.dataset_name}/" \
                                  f"{configuration}/unet"
        ...
        # self.output_folder_base = join(self.output_folder_base, f"wave_unet")
        ...
        print(f"self.output_folder ==================== {self.output_folder}")
    ...
    def build_network_architecture(plans_manager: PlansManager,
        ...
        return get_network_from_plans(plans_manager, dataset_json, configuration_manager,
                                      ...
                                      base_ch=LiverTrainer.base_ch,
                                      block=LiverTrainer.block,
                                      use_my_unet=LiverTrainer.use_my_unet,
                                      setting=LiverTrainer.setting)
    ...
    def run_training(self, dataset_id):
        ...
        if not self.debug and self.local_rank == 0:
            ...
            self.initialize_wandb(project=f"{self.dataset_name}_{self.fold}",
                                  # name=f"{self.network.__class__.__name__}"
                                  #      f"_lr_{self.initial_lr}",
                                  name="Liver",
    ...
    def real_validation_brain(self):
        ...
        def dice_score_vol(y_pred, y_true, num_classes, voxel_spacing):
            ...
            cancer_true = y_true == 2
            cancer_pred = y_pred == 2

            liver_dsc = dc(liver_pred, liver_true)
            try:
                liver_hd95 = medpy.metric.hd95(liver_pred, liver_true,
                                               voxelspacing=voxel_spacing)
            except:
                print(f"================ found liver problem: y_pred.max(), {y_pred.max()}, y_true.max() {y_true.max()}")
                liver_hd95 = np.nan

            cancer_dsc = dc(cancer_pred, cancer_true)
            try:
               cancer_hd95 = medpy.metric.hd95(cancer_pred, cancer_true,
                                               voxelspacing=voxel_spacing)
            except:
                print(f"================ found cancer problem: y_pred.max(), {y_pred.max()}, y_true.max() {y_true.max()}")
                cancer_hd95 = np.nan

            dice = [liver_dsc, cancer_dsc]
            case_hd95 = [liver_hd95, cancer_hd95]

            return dice, case_hd95
            # print(dice, case_hd95)
            # return ReturnRes(dice=dice, hd95=case_hd95)
        ...
        def get_score(file_ref, file_pred, ref_reader, pred_reader):
            ...
            return case_vol_dice, case_hd95
        ...
        results = Parallel(-1)(delayed(get_score)(file_ref, file_pred, ref_reader, pred_reader)
        ...
        for res in results:  # dice, hd95
            # all_dice_slice_level.append(res[0])  # [(C1, 4), (C2, 4) ...]
            all_haus95.append(res[1])
            all_dice_vol_level.append(res[0])  # [(4,), (4,) ...]
        ...
        dsc_v_mean = np.mean(dsc_v)

        haus95_mean = np.nanmean(haus95, axis=0)
        ...
        if self.local_rank == 0:
            ...
            if not self.debug:
                ...
                """
                    ...
                    TC: tumor core (non-enhancing tumor + enhancing tumor) (2+3)
                    WT: whole tumor (non-enhancing tumor + enhancing tumor + edema) (1+2+3)
                """
                wandb.log(data={"test/liver_dsc": dsc_v[0]}, step=self.current_epoch)
                wandb.log(data={"test/cancer_dsc": dsc_v[1]}, step=self.current_epoch)
                # wandb.log(data={"test/WT_dsc": dsc_v[2]}, step=self.current_epoch)

                if len(haus95) > 0:
                    wandb.log(data={"test/liver_hau": haus95_mean[0]}, step=self.current_epoch)
                    wandb.log(data={"test/cancer_hau": haus95_mean[1]}, step=self.current_epoch)
                # wandb.log(data={"test/WT_hau": haus95_mean[2]}, step=self.current_epoch)
    ...
    def initialize(self):
        # self.initialize_wandb(name="Liver", project="Liver")
