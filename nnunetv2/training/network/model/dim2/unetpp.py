...
from unet_v2.utils import get_block


class UNetPlusPlus(nn.Module):
    def __init__(self, in_ch, num_classes, base_ch=32,
                 block='SingleConv', deep_supervision=True):
        super().__init__()
        ...
        self.seg_outputs = nn.ModuleList([
            nn.Conv2d(n_ch[0], num_classes, kernel_size=1)
        ] * 4)
    ...
    def forward(self, x):
        ...
        if self.deep_supervision:
            return [
                self.seg_outputs[3](x0_4),
                self.seg_outputs[2](x0_3),
                self.seg_outputs[1](x0_2),
                self.seg_outputs[0](x0_1)
            ]
        else:
...
if __name__ == "__main__":
    ...
    ys = model(torch.rand(1, 3, 256, 256))
    for y in ys:
        print(y.shape)
    times = []
    import time
