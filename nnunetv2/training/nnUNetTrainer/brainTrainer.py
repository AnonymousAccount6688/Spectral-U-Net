...
from nnunetv2.utilities.brain_get_network_from_plans import get_network_from_plans
...
class BrainTrainer(nnUNetTrainer):
    base_ch = 16
    ...
    def build_network_architecture(plans_manager: PlansManager,
        ...
        return get_network_from_plans(plans_manager, dataset_json, configuration_manager,
                                      ...
                                      base_ch=BrainTrainer.base_ch,
                                      block=BrainTrainer.block,
                                      use_my_unet=BrainTrainer.use_my_unet,
                                      setting=BrainTrainer.setting)
    ...
    def run_training(self, dataset_id):
        ...
        if not self.debug and self.local_rank == 0:
            ...
            self.initialize_wandb(project=f"{self.project_prefix}",  # f"{self.project_prefix}_{self.fold}"
                                  name=f"{str(self.network.__class__)}"
    ...
    def real_validation_brain(self):
        ...
        def dice_score_vol(y_pred, y_true, num_classes):
            ...
            tc_true = np.logical_or(y_true == 2, y_true == 3)
            tc_pred = np.logical_or(y_pred == 2, y_pred == 3)

            wt_true = y_true != 0
            wt_pred = y_pred != 0

            et_dsc = dc(et_pred, et_true)
            et_hd95 = medpy.metric.hd95(et_pred, et_true, voxelspacing=(1, 1, 1))

            tc_dsc = dc(tc_pred, tc_true)
            tc_hd95 = medpy.metric.hd95(tc_pred, tc_true, voxelspacing=(1, 1, 1))

            wt_dsc = dc(wt_pred, wt_true)
            wt_hd95 = medpy.metric.hd95(wt_pred, wt_true, voxelspacing=(1, 1, 1))

            dice = [et_dsc, tc_dsc, wt_dsc]
            case_hd95 = [et_hd95, tc_hd95, wt_hd95]
        ...
        all_dice_vol_level = np.array(all_dice_vol_level)  # (N, 3)

        dsc_v = np.mean(all_dice_vol_level, axis=0)  # (3, )

        haus95 = np.array(all_haus95)  # (N, 3)
        ...
        if self.local_rank == 0:
            ...
            if not self.debug:
                ...
                """
                    ...
                    WT: whole tumor (non-enhancing tumor + enhancing tumor + edema) (1+2+3)
                """
                wandb.log(data={"test/ET_dsc": dsc_v[0]}, step=self.current_epoch)
                wandb.log(data={"test/TC_dsc": dsc_v[1]}, step=self.current_epoch)
                wandb.log(data={"test/WT_dsc": dsc_v[2]}, step=self.current_epoch)

                wandb.log(data={"test/ET_hau": haus95_mean[0]}, step=self.current_epoch)
                wandb.log(data={"test/TC_hau": haus95_mean[1]}, step=self.current_epoch)
    ...
    def initialize(self):
        # self.initialize_wandb(name="Brain", project="Brain")
        if not self.was_initialized:
            ...
            #                                f"{self.job_id}_{self.network.__class__.__module__}."
            #                                f"{self.network.__class__.__name__}")
